...
func tableSplunkApp(ctx context.Context) *plugin.Table {
	return &plugin.Table{
		...
		Columns: []*plugin.Column{
			...
			{Name: "managed_by_deployment_client", Type: proto.ColumnType_BOOL, Transform: transform.FromField("Content.ManagedByDeploymentClient"), Description: "If true, app is managed by a deployment client."},
			{Name: "show_in_nav", Type: proto.ColumnType_BOOL, Transform: transform.FromField("Content.ShowInNav"), Description: "If true, app is navigable from Splunk Web."},
			{Name: "state_change_requires_restart", Type: proto.ColumnType_BOOL, Transform: transform.FromField("Content.StateChangeRequiresRestart"), Description: "Indicates whether restart required on state change."},
